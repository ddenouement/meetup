...
public class SpeakerController {
    ...
    public ResponseEntity<Meeting> createMeeting(@CookieValue(value = "token", defaultValue = "") String token,
                                        @RequestBody Meeting meeting) {
        try {
            return new ResponseEntity<>(meetingService.createMeeting(meeting, token),HttpStatus.CREATED);
    ...
    public ResponseEntity<List<Meeting>> getMyMeetings(@CookieValue(value = "token", defaultValue = "") String token) {
        try {
            return new ResponseEntity<>(meetingService.getSpeakerMeetings(token), HttpStatus.OK);
        } catch (IllegalAccessException | NullPointerException | NoSuchElementException ex) {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
        }
    }
